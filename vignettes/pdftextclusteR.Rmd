...
Below is an example of how to import a PDF file using the pdftools package, then detect columns with `detect_clusters()`, plot the results with `plot_clusters()` andd finally extract the text with `extract_clusters()` function.
...
Clusters - usually columns and text boxes - are detected with the `detect_clusters()` function.
...
The text of the detected clusters can be extracted with the `extract_clusters()`. 
...
```{r}
ka[[7]] |> 
  detect_clusters(algorithm = "sNNclust") |> 
  plot_clusters()
```
